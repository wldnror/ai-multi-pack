...
def shock_event_handler(file_path):
    ...
    while True:
        ...
        if acceleration_data['x'] > threshold or acceleration_data['y'] > threshold or acceleration_data['z'] > threshold:
            # 충격 감지 시간 저장
            shock_time = time.strftime("%Y-%m-%d_%H-%M-%S")
            # 가장 최근에 생성된 녹화 파일 찾기
            latest_recording = max([f for f in os.listdir(file_path) if f.endswith('.mp4')], key=os.path.getctime)
            current_recording = os.path.join(file_path, latest_recording)
            new_recording = os.path.join(os.path.dirname(file_path), 'shock_videos', f'shock_video_{shock_time}.mp4')
