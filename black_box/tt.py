...
def shock_event_handler(file_path):
    ...
    while True:
        ...
        if acceleration_data['x'] > threshold or acceleration_data['y'] > threshold or acceleration_data['z'] > threshold:
            ...
            current_recording = os.path.join(file_path, f'video_{current_time}.mp4')
            new_recording = os.path.join(os.path.dirname(file_path), 'shock_videos', f'shock_video_{current_time}.mp4')
