...
left_active = False
right_active = False
last_gyro_y = None  # 이전 자이로스코프 값 저장을 위한 변수
...
def main():
    args = parse_args()
    global manual_mode, left_active, right_active, last_gyro_y
    ...
    try:
        while True:
            if not manual_mode:
                ...
                if last_gyro_y is not None:
                    delta_gyro_y = gyro_y - last_gyro_y
                    if delta_gyro_y < -50:  # 임계값 설정, 예를 들어 -50
                        left_active = True
                        right_active = True
                    elif angle_y > 20:
                        right_active = True
                ...
                print(f"Gyro Y-axis speed: {gyro_y}, Change in gyro: {delta_gyro_y if 'delta_gyro_y' in locals() else 'N/A'}, Tilt angle Y-axis: {angle_y}")
