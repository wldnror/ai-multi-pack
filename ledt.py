...
def sweep_panel(start_index, end_index, color):
    for i in range(start_index, end_index, 5):
        for j in range(num_pixels_per_panel):
            pixels[(i + j) % num_pixels_per_panel + start_index] = color if j < 5 else (0, 0, 0)
        pixels.show()
        time.sleep(0.1)
...
def control_leds():
    ...
    while True:
        ...
        if accel_x > 1500:
            # 우측 기울기: 빨간색 LED 패널 활성화 및 스윕
            sweep_panel(0, num_pixels_per_panel, (255, 0, 0))
        elif accel_x < -1500:
            # 좌측 기울기: 파란색 LED 패널 활성화 및 스윕
